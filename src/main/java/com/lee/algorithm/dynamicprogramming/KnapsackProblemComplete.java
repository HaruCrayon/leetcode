...
//                            剩余空间能装的最大价值 + 当前物品价值
    ...
    private static void print(int[][] dp) {
        System.out.println("   " + String.join("", Collections.nCopies(63, "-")));
        Object[] array = IntStream.range(0, dp[0].length + 1).boxed().toArray();
        System.out.printf(String.join("", Collections.nCopies(dp[0].length, "%5d ")) + "%n", array);
        for (int[] d : dp) {
            array = Arrays.stream(d).boxed().toArray();
            System.out.printf(String.join("", Collections.nCopies(d.length, "%5d ")) + "%n", array);
        }
    }
